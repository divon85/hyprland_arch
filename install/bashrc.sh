# ------------------------------------------------------
# Install .bashrc
# ------------------------------------------------------

if [ ! -L ~/.bashrc ] && [ -f ~/.bashrc ]; then
    echo "PLEASE NOTE: The script has detected an existing .bashrc file."
fi
if [ -f ~/.config-versions/backups/$datets/.bashrc-old ]; then
    echo "Backup is already available here ~/.config-versions/backups/$datets/.bashrc-old"
    echo
fi
if [ -f ~/.config-versions/$version/.bashrc ] ;then
    echo -e "${GREEN}"
    figlet ".bashrc"
    echo -e "${NONE}"
    if [ -f ~/.bashrc ] ;then
        echo ":: The script has detected an existing .bashrc"
        echo ":: You can replace it with the latest edited .bashrc."
        echo
        if gum confirm "Do you want to replace your existing .bashrc?" ;then
            echo ":: .bashrc will be installed"
        elif [ $? -eq 130 ]; then
                exit 130
        else
            rm ~/.config-versions/$version/.bashrc
            echo ":: Installation of the .bashrc file skipped."
        fi
    elif [ ! -L ~/.bashrc ] && [ -f ~/.bashrc ]; then
        echo ":: The script has detected an existing .bashrc file."
        if [ -f ~/.config-versions/backups/$datets/.bashrc-old ]; then
            echo "Backup is already available here ~/.config-versions/backups/$datets/.bashrc-old"
            echo
        fi
        echo ":: You can replace it with the latest version of ML4W Dotfiles $version."
        echo
        if gum confirm "Do you want to replace your existing .bashrc?" ;then
            rm ~/.bashrc
            echo ":: .bashrc will be installed"
        elif [ $? -eq 130 ]; then
                exit 130
        else
            echo ":: Installation of the .bashrc file skipped."
        fi
    fi
    echo
fi
